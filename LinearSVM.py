...
def initialize_models():
    ...
    Returns:
        LinearSVC: Initialized LinearSVC model.
    """
    lsvc = LinearSVC(dual=False)  # Explicitly setting dual=False to suppress FutureWarning
    calibrated_SVC = CalibratedClassifierCV(lsvc, method='sigmoid', cv=5)
...
def compute_metrics(y_test, model_pred, model_prob):
    ...
    Args:
        ...
        model_prob (array): Predicted probabilities for AUC computation.
    ...
    if len(np.unique(y_test)) == 2:
        auc = roc_auc_score(y_test, model_prob[:, 1])
    else:
...
def main(data_path):
    ...
    for train_index, test_index in rsfk.split(X_data, y_data):
        ...
        accuracy, f1, precision, recall, auc = print_metrics(y_test, lsvc_pred, lsvc_prob, 'Linear SVC')
    ...
    if aucs:
        ...
        avg_auc = std_auc = None

    # Save metrics to a text file
    with open("resultsSVMXception.txt", "w") as file:
